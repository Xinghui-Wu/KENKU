        ...
        for feature_parameters_dict in feature_parameters:
            ...
            for song_filename in song_filenames:
                ...
                for origin in range(0, song.size()[1] - command.size()[1] + 1, int(16000 * interval)):
                    ...
                    integrated_command_path = os.path.join(integrated_command_dir, 
                                                           "{}-{}-{}-{}-{}-{}-{}-{}-{}-{}.wav".format(command_name, song_filename, time_origin, 
                    ...
                    song_clip_path = os.path.join(integrated_command_dir, 
                                                  "{}-{}-{}-{}-{}-{}-{}-{}-{}-{}.wav".format(command_name, song_filename, time_origin, 
                                                                                             feature_parameters_dict.get("n_mfcc", ""), 
                                                                                             feature_parameters_dict.get("n_mels", ""), 
                                                                                             feature_parameters_dict.get("n_fft", ""), 
                    ...
                    attack_sample(feature_extractor, command_info[0], song_path, integrated_command_path, song_clip_path, origin, 
                                  penalty, optimizer, learning_rate, num_iterations)

                    integrated_command_table.append([integrated_command_path, command_info[1]])
...
def attack_sample(feature_extractor, command_path, song_path, integrated_command_path, song_clip_path, origin, penalty, optimizer, learning_rate, num_iterations):
    ...
    logging.info("loss_feature = {:.4f}, loss_delta = {:.4f}, loss = {:.4f}".format(torch.norm(integrated_command_feature - command_feature).data, 
                                                                                    torch.norm(delta).data, 
                                                                                    torch.norm(integrated_command_feature - command_feature).data + penalty * torch.norm(delta).data))
    ...
    for i in range(num_iterations):
        optimizer.zero_grad()
        integrated_command_feature = feature_extractor(waveform=song+delta)
        loss = torch.norm(integrated_command_feature - command_feature) + penalty * torch.norm(delta)
        loss.backward()
        optimizer.step()
