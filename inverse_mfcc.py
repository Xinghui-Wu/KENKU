...
def attack(commands, malicious_samples, transcriptions, num_iterations):
    """An attack method based on Griffin-Lim to reverse MFCC
    ...
    Args:
        ...
        num_iterations (int): The number of iterations of inverse MFCC.
    """
    ...
    for i in range(len(command_filenames)):
        ...
        target_command_mfcc = mfcc(y=command, sr=16000)

        for j in range(num_iterations):
            print(command_filenames[i], j)

            command_mfcc = mfcc(y=command, sr=16000)
            command = mfcc_to_audio(mfcc=command_mfcc)

            print("||malicious_sample_mfcc - command_mfcc|| = {:.4f}".format(np.linalg.norm(mfcc(y=command, sr=16000) - target_command_mfcc)))
...
if __name__ == '__main__':
    ...
    parser.add_argument("-n", "--num_iterations", type=int, default=5, help="The number of iterations of inverse MFCC.")
